...
const Model = forwardRef<HTMLDivElement, ModelProps>(({ title, isOpen, className, children, onClick }, ref) => {
    return (
        <div className={`absolute w-full h-full z-30 top-0 left-0 bg-black/15 transition-all duration-200 ease-in-out ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"} ${className}`}>
            <div ref={ref} className={`duration-200 ease-in-out ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"} absolute left-1/2 -translate-x-1/2 bottom-2/3 translate-y-1/2 max-md:w-11/12 md:min-w-3xs p-4 rounded-2xl dark:bg-dark-background-tertiary bg-light-background-tertiary drop-shadow-2xl border dark:border-dark-border-tertiary border-light-border-secondary`}>
