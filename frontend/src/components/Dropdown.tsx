...
const Dropdown = ({ children, absolute = false, isOpen }: DropdownProps) => {
  ...
  const wrapperClass = `${absolute ? "absolute top-full translate-y-2 right-0 z-20 w-full" : "py-1 pr-1"} transform transition-all duration-200 ease-out overflow-hidden rounded-lg`
  const visibleState = isOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-1 pointer-events-none";

  const innerClass = `${absolute ? "p-2 dark:bg-dark-background-tertiary bg-light-background-tertiary drop-shadow-2xl" : ""} space-y-1 *:w-full`;
